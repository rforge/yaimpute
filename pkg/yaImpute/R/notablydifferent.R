notablyDifferent <- function (object,vars=NULL,threshold=NULL,p=.05,...)
{
   ...
   vo <- paste(both,"o",sep=".")
   diff <- (impObj[,both]-impObj[,vo,drop=FALSE])/
             unlist(lapply(impObj[,vo,drop=FALSE],sd))
   rmsd <- sqrt(apply(diff * diff,1,mean))
   if (is.null(threshold)) threshold <- quantile(rmsd[refIds],1-p)
   ans <- list(notablyDifferent.refs=sort(rmsd[refIds][rmsd[refIds]>threshold]),
...
{ 
  ...
  if (class(x) == "list")
  {
    ...
    for (i in 1:length(x))
    {
      ...
      cy <- par()$cxy[2]
      cy <- cy*1.1
      if (i == 1) text(x=.05*xlim[2],y=.99*ylim[2],pos=4,"Legend")
      text(x=.05*xlim[2],y=(.99*ylim[2])-(i+2)*cy,pos=4,
  ...
  { 
    ...
    if (!add)
    {
      cxy = par()$cxy
      cxy = cxy*1.1
      bxy = c(.05*length(all),.99*max(all))
